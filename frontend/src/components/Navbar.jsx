    ...
    <nav className="w-full bg-conic/increasing from-violet-700 via-lime-300 to-violet-700 shadow-md px-4 py-3 flex items-center justify-between">
      ...
      <div className="flex-1 mx-4 max-w-xl">
        <input
          ...
          className={`${theme === 'light' ? 'light' : 'dark'} dark:bg-gray-800 dark:text-white bg-white w-full border rounded-full px-4 py-2 shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-400`}
        />
      </div>
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          {user ? <>
                     <span className="px-4 py-2">{user._doc.name}</span>
                    <button
                      onClick={logout}
